...
<div class="row">
    ...
    <div class="col-md-6">
        <div >
            ...
            <input type="file" name="{{$input}}" class="mb-3">
            @error($input)
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
             </span>
    ...
    <div class="col-md-6">
        <div class="form-group bmd-form-group">
            ...
            <select  name="{{$input}}" class="form-control @error($input) is-invalid @enderror">
                <option style="background-color: #0b3251" value="1" {{ isset($users) && $users->{$input} == 1 ? 'selected'  :'' }}>published</option>
                <option style="background-color: #0b3251" value="0" {{ isset($users) && $users->{$input} == 0 ? 'selected'  :'' }}>hidden</option>
            </select>
            @error($input)
            <span class="invalid-feedback" role="alert">
                   <strong>{{ $message }}</strong>
        ...
        <div class="form-group bmd-form-group">
            ...
            <select  name="{{$input}}" class="form-control @error($input) is-invalid @enderror">
                @foreach($categories  as $caegory)
                    <option style="background-color: #0b3251" value="{{ $caegory->id }}" {{ isset($users) && $users->{$input} == $caegory->id ? 'selected'  :'' }}>{{ $caegory->name }}</option>
                @endforeach
            </select>
            @error($input)
            <span class="invalid-feedback" role="alert">
